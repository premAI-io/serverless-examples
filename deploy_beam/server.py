...
app = App(
    ...
    runtime=Runtime(
        cpu=CPU,
        gpu=GPU,
        image=Image(python_version=PYTHON_VERSION, python_packages=PACKAGES),
    ),
...
def web_server(**inputs):
    ...
    async def _stream(
        chat_input: str, generation_kwargs: dict
        ...
        gen_kwargs = dict(
            input_ids=input_ids, streamer=streamer, **generation_kwargs
        )
